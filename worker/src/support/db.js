...
export async function getQuickFaqs(db) {
...
export async function getFaqByIntent(db, intent) {
...
export async function getKnowledgeBase(db) {
  const { results } = await db.prepare(
    "SELECT title, content FROM knowledge_base WHERE locale = 'en'"
  ).all();
  return results || [];
}
...
export async function checkRateLimit(db, key, limit, windowSec) {
  try {
    ...
    console.error("[support rate limit]", err);
    return { ok: true };
  }
}
